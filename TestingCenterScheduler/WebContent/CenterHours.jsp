			...
			<div class="container-fluid">
				...
				<div>
					<div class="col-lg-12">
						...
						<ol class="breadcrumb">
							<li class="active"><a href="Admin.jsp">Home</a> > Testing
					...
					<div class="col-sm-6">
						<select class="form-control" id="select">
							<c:forEach items="${infolist}" var="info" varStatus="i">
								<option value="${i.index}">${info.term.termID}
									${info.term.termSeason}_${info.term.termYear}</option>
							</c:forEach>
				...
				<c:forEach items="${infolist}" var="info" varStatus="i">
					<!-- /.row -->
					<div class="row formTable" id="table${i.index}"
						style="display: none;">
						<div class="col-lg-12">
							<form action="AdministratorHome" method="POST">
								<table class="info-Table">
									<tr>
										<th>Term:</th>
										<td><input type="text" class="form-control" id="term"
											name="term" value="${info.term.termID}" readonly /></td>
									</tr>
									<tr>
											...
											<table class="info-table" style="width: 100%">
												...
												<tr>
													...
													<td>
														<div class="form-inline">
															<div class="form-group">
																<label class="sr-only" for="sato"></label> <select
																	name="sato" class="form-control" style="width: 100px">
																	<c:if test="${sath[0] != 'Closed'}">
																		<c:forEach begin="0" end="23" var="val">
																			<option
																				${(satohr == val) && (satomin == "00") ? 'selected' : ''}>
																				${val}:00</option>
																			<option
																				${(satohr == val) && (satomin == "30") ? 'selected' : ''}>
																				${val}:30</option>
																		</c:forEach>
																		<option>Closed</option>
																	</c:if>
																...
																</select> to <label class="sr-only" for="satc"></label> <select
																	name="satc" class="form-control" style="width: 100px">
																	<c:if test="${sath[1] != 'Closed'}">
																		<c:forEach begin="0" end="23" var="val">
																			<option
																				${(satchr == val) && (satcmin == "00") ? 'selected' : ''}>
																				${val}:00</option>
																			<option
																				${(satchr == val) && (satcmin == "30") ? 'selected' : ''}>
																				${val}:30</option>
																		</c:forEach>
																		<option>Closed</option>
																	</c:if>
												...
												<tr>
													...
													<td>
														<div class="form-inline">
															<div class="form-group">
																<label class="sr-only" for="suno"></label> <select
																	name="suno" class="form-control" style="width: 100px">
																	<c:if test="${sunh[0] != 'Closed'}">
																		<c:forEach begin="0" end="23" var="val">
																			<option
																				${(sunohr == val) && (sunomin == "00") ? 'selected' : ''}>
																				${val}:00</option>
																			<option
																				${(sunohr == val) && (sunomin == "30") ? 'selected' : ''}>
																				${val}:30</option>
																		</c:forEach>
																		<option>Closed</option>
																	</c:if>
																...
																</select> to <label class="sr-only" for="sunc"></label> <select
																	name="sunc" class="form-control" style="width: 100px">
																	<c:if test="${sunh[1] != 'Closed'}">
																		<c:forEach begin="0" end="23" var="val">
																			<option
																				${(sunchr == val) && (suncmin == "00") ? 'selected' : ''}>
																				${val}:00</option>
																			<option
																				${(sunchr == val) && (suncmin == "30") ? 'selected' : ''}>
																				${val}:30</option>
																		</c:forEach>
																		<option>Closed</option>
																	</c:if>
										...
										<td>
											<div class="form-group">
												<label class="sr-only" for="gaptime">Gap Time:</label> <input
													...
													name="gaptime" min="0" max="30" placeholder="Minutes"
													value="${info.gapTime}" required>
											</div>
										</td>
									</tr>
									<tr>
										<th>Closing Dates:</th>
										<td><input type="text" class="form-control" id="closing"
											name="closing" value="${info.closedDate}" /></td>
									</tr>
									<tr>
										<th>Reserve Time:</th>
										<td><input type="text" class="form-control" id="reserve"
											name="reserve" value="${info.reserveTime}" /></td>
									</tr>
									<tr>
										<th>Reminder Interval:</th>
										<td><input type="text" class="form-control" id="reminder"
											name="reminder" value="${info.reminderInterval}" required /></td>
									</tr>
									<tr>
										<th>Seats:</th>
										<td><input type="text" class="form-control" id="seat"
											name="seat" value="${info.seats}" required /></td>
									</tr>
									<tr>
										<th>Set-a-side Seats:</th>
										<td><input type="text" class="form-control" id="setseat"
											name="setseat" value="${info.setAsideSeats}" required /></td>
									</tr>
								</table>
						...
						<form action="AdministratorHome" method="POST">
							<table class="info-Table">
								<tr>
									<th>Term:</th>
									<td><input type="text" class="form-control" id="term"
										name="term" placeholder="Enter term" required /></td>
								</tr>
								<tr>
									<th>Open Hours:</th>
									<td>
								...
								<tr>
									<th>Closing Dates:</th>
									<td><input type="text" class="form-control" id="closing"
										name="closing" placeholder="Enter closing date range" /></td>
								</tr>
								<tr>
									<th>Reserve Time:</th>
									<td><input type="text" class="form-control" id="reserve"
										name="reserve" placeholder="Enter reserve time" /></td>
								</tr>
								<tr>
									<th>Reminder Interval:</th>
									<td><input type="text" class="form-control" id="reminder"
										name="reminder" placeholder="Enter reminder interval" required /></td>
								</tr>
								<tr>
									<th>Seats:</th>
									<td><input type="text" class="form-control" id="seat"
										name="seat" placeholder="Enter seats" required /></td>
								</tr>
								<tr>
									<th>Set-a-side Seats:</th>
									<td><input type="text" class="form-control" id="setseat"
										name="setseat" placeholder="Enter seat aside seats" required /></td>
								</tr>
							</table>
							<input type="submit" class="btn btn-primary" name="edit"
								value="Submit" />
